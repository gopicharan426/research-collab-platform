...
function createPost($userId, $title, $description) {
    ...
    if ($stmt->execute([$userId, $title, $description])) {
        ...
        $s->execute([$userId]);
        $author = $s->fetch(PDO::FETCH_ASSOC);
...
function getAllPosts($search = '', $currentUserId = 0) {
    $pdo = getDBConnection();
    $fp  = $currentUserId ? "CASE WHEN f.follower_id = $currentUserId THEN 0 ELSE 1 END" : "1";
    if (!empty($search)) {
        $s = '%' . $search . '%';
        $stmt = $pdo->prepare("SELECT p.post_id, p.title, p.description, p.created_at, p.views, u.name as author_name, u.user_id as author_id, u.username as author_username, $fp as feed_priority FROM research_posts p JOIN users u ON p.user_id = u.user_id LEFT JOIN followers f ON f.following_id = p.user_id AND f.follower_id = ? WHERE p.title LIKE ? OR p.description LIKE ? ORDER BY feed_priority ASC, p.created_at DESC");
        $stmt->execute([$currentUserId, $s, $s]);
    } else {
        $stmt = $pdo->prepare("SELECT p.post_id, p.title, p.description, p.created_at, p.views, u.name as author_name, u.user_id as author_id, u.username as author_username, $fp as feed_priority FROM research_posts p JOIN users u ON p.user_id = u.user_id LEFT JOIN followers f ON f.following_id = p.user_id AND f.follower_id = ? ORDER BY feed_priority ASC, p.created_at DESC");
...
function deletePost($postId, $userId, $isAdmin = false) {
    ...
    if ($isAdmin) {
        ...
        if ($post && $post['user_id'] == $userId) {
            $pdo->prepare("DELETE FROM research_posts WHERE post_id = ?")->execute([$postId]);
            return "success";
        }
    }
