...
function updateUserProfile($userId, $data) {
    ...
    if (!empty($username)) {
        if (!preg_match('/^[a-zA-Z0-9_]{3,30}$/', $username)) return "Username must be 3-30 characters.";
        $stmt = $pdo->prepare("SELECT user_id FROM users WHERE username = ? AND user_id != ?");
        $stmt->execute([$username, $userId]);
        if ($stmt->fetch()) return "Username already taken.";
    } else { $username = null; }
    ...
    if ($stmt->execute([$username, htmlspecialchars(trim($data['bio'] ?? ''), ENT_QUOTES, 'UTF-8'), htmlspecialchars(trim($data['phone'] ?? ''), ENT_QUOTES, 'UTF-8'), htmlspecialchars(trim($data['location'] ?? ''), ENT_QUOTES, 'UTF-8'), $website, $linkedin, htmlspecialchars(trim($data['research_interests'] ?? ''), ENT_QUOTES, 'UTF-8'), $userId])) return "success";
